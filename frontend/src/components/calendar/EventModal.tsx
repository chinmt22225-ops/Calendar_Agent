...
function plusMonth(value: string) { const date = new Date(`${value.slice(0, 10)}T12:00:00`); date.setMonth(date.getMonth() + 1); return localInputValue(date).slice(0, 10) }
...
export function EventModal({ event, initialRange, categories, timeZone, onClose, onSave, onDelete }: {
  ...
  const save = async () => {
    ...
    if (!draft.all_day && new Date(draft.end_time) <= new Date(draft.start_time)) { setError('Thời gian kết thúc phải sau thời gian bắt đầu.'); return }
  ...
  return (
    <div className="modal-backdrop" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
      <section className="event-modal">
        <header><div><span className="modal-icon"><CalendarClock size={20} /></span><div><h2>{event ? 'Chỉnh sửa sự kiện' : 'Tạo sự kiện mới'}</h2><p>{event?.is_ai_generated ? 'Do Planora sắp xếp · Bạn vẫn có thể chỉnh sửa' : 'Thêm vào lịch học của bạn'}</p></div></div><button aria-label="Đóng" title="Đóng" onClick={onClose}><X size={19} /></button></header>
        <div className="modal-body">
          ...
          <div className="field-row">
            <label className="field"><span><Clock3 size={14} /> {draft.all_day ? 'Từ ngày' : 'Bắt đầu'}</span><input type={inputType} value={draft.start_time} onChange={(e) => setDraft({ ...draft, start_time: e.target.value })} /></label>
            <label className="field"><span><Clock3 size={14} /> {draft.all_day ? 'Đến hết ngày' : 'Kết thúc'}</span><input type={inputType} value={draft.end_time} onChange={(e) => setDraft({ ...draft, end_time: e.target.value })} /></label>
          </div>
          ...
          {draft.recurrence_rule && draft.recurrence_end && <p className="recurrence-summary"><Repeat2 size={13} /> Lặp lại {draft.recurrence_rule === 'daily' ? 'hằng ngày' : draft.recurrence_rule === 'weekly' ? 'hằng tuần' : 'hằng tháng'} đến hết ngày {new Date(`${draft.recurrence_end}T12:00:00`).toLocaleDateString('vi-VN')}.</p>}
